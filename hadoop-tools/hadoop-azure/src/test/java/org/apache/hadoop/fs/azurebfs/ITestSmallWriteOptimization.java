...
public class ITestSmallWriteOptimization extends AbstractAbfsScaleTest {
  ...
  private void formulateSmallWriteTestAppendPattern(final AzureBlobFileSystem fs,
    ...
    while (testIteration > 0) {
      ...
      int totalAppendFlushCalls = (flushWillBeMergedWithAppend
          ...
          : (wasDataPendingToBeWrittenToServer)
              ...
              : (recurringWriteSize == 0 && !OperativeEndpoint.OperativeEndpointFallback.isIngressEnabledOnDFS(prefixMode, fs.getAbfsStore().getAbfsConfiguration()))
    ...
    opStream.close();
    if (OperativeEndpoint.OperativeEndpointFallback.isIngressEnabledOnDFS(prefixMode, fs.getAbfsStore().getAbfsConfiguration())) {
